"""Dandân — {U}{U} Creature — Fish 4/1.
This creature can't attack unless defending player controls an Island.
When you control no Islands, sacrifice this creature.

The attack restriction depends on the opponent's Islands (never any → it can't
attack). The "sacrifice if you control no Islands" clause is checked at your upkeep
(approximating the state trigger)."""
...
class Dandan(Card):
    ...
    def on_phase(self, state, perm, phase):
        if not any(p.is_land and "island" in p.type_line.lower() for p in state.battlefield):
            ...
            if p is not None:
                state.emit("Dandân: no Island — sacrifice")
